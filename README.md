<div align="center">

#  Hi, I'm Ali Riza Sevgili

### IT Risk & GRC Analyst | PCI DSS • IAM • Data Privacy

Bridging Law (LL.B.) and Technology to identify risks, implement security controls, and support compliance.

<p align="center">
  <a href="https://www.linkedin.com/in/alirizasevgili/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:alirizasevgili@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

</div>

---

##  Focus

- IT Risk & Control Implementation  
- PCI DSS & Payment Security  
- Identity & Access Management (IAM)  
- Data Privacy (GDPR / KVKK)  
- Secure SDLC & Change Management  

---

##  Selected Work

### PCI DSS Risk Assessment (In Progress)
- Identifying risks in e-commerce payment flows  
- Mapping PCI DSS controls to mitigate CHD exposure  
- Improving compliance and audit readiness  

---

### IAM Control Implementation
- Designed access control model for PII systems  
- Applied least privilege and data masking techniques  
- Reduced risk of unauthorized access  

---

### Secure SDLC (Heart Pottery Project)
- Implemented change management via CI/CD  
- Used Git as audit trail for control validation  
- Applied OWASP Top 10 mitigations  

---

##  Technical & Framework Knowledge

- PCI DSS  
- OWASP Top 10  
- IAM (JWT, OAuth)  
- Secure SDLC  
- NIST CSF (familiarity)  
- ISO 27001 (familiarity)  

---

##  Goal

Seeking entry-level roles in:
- IT Risk Analyst  
- GRC / Compliance Analyst  
- Information Security Governance  

---

<div align="center">
  <i>Focused on reducing risk, implementing controls, and supporting compliance.</i>
</div>
